import type { Config, Package, Service } from "./types";

// Baked-in defaults so the site renders before (or without) a Convex deployment.
// The Convex `seed:all` mutation seeds the same content — keep these in sync.
...
const rawServices: Array<Omit<Service, "sortOrder" | "isActive">> = [
  // Massage
  {
...
export const defaultServices: Service[] = rawServices.map((s, i) => ({
  ...s,
  sortOrder: (i + 1) * 10,
  isActive: true,
}));

const rawPackages: Array<Omit<Package, "sortOrder" | "isActive">> = [
  {
    slug: "villa-reset",
...
export const defaultPackages: Package[] = rawPackages.map((p, i) => ({
  ...p,
  sortOrder: (i + 1) * 10,
  isActive: true,
}));
